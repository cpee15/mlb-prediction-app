...
function UsersPanel({ data, currentUserId, onRefresh }) {
  ...
  return <div style={s.stack}>
    <section style={s.sectionHeader}><div><div style={s.eyebrow}>Safe Directory</div><h2 style={s.sectionTitle}>Users & Access</h2><p style={s.copy}>Edit directory metadata, account state, and role assignment. Role changes are server-owned, audited, and require the user to sign in again.</p></div><Badge>{data?.totalSize || 0} users</Badge></section>
    ...
    {selected ? <form style={s.card} onSubmit={saveUser}>
      ...
      <div style={s.checkRow}><label><input type="checkbox" checked={editor.is_active !== false} disabled={selected.id === currentUserId} onChange={event => setEditor(current => ({ ...current, is_active: event.target.checked }))} /> Active</label><label><input type="checkbox" checked={Boolean(editor.is_locked)} disabled={selected.id === currentUserId} onChange={event => setEditor(current => ({ ...current, is_locked: event.target.checked }))} /> Locked</label><label><input type="checkbox" checked={Boolean(editor.is_admin)} disabled={selected.id === currentUserId} onChange={event => setEditor(current => ({ ...current, is_admin: event.target.checked }))} /> Administrator</label><Badge>{editor.is_admin ? 'Administrator' : titleCase(selected.profile_key)}</Badge></div>
