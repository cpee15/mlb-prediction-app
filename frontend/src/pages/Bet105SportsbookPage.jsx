...
function marketDisplayKey(market) {
  return [market?.market_id || '', marketKey(market), market?.line_key || market?.line || 'none', asArray(market?.selections).map(selection => `${selection?.selection_id || selection?.name || selection?.team}:${selection?.price}`).join('|')].join('::')
}
...
function categoryFor(market) {
  ...
  if (key.includes('batter') || key.includes('hitting')) return 'Batter Props'
  if (key.includes('pitcher')) return 'Pitcher Props'
  if (key.includes('inning') || key.includes('period')) return 'Innings / Periods'
  if (key.includes('team')) return 'Team Props'
...
function onlyMoneyline(event) {
  const markets = asArray(event?.markets).filter(market => asArray(market?.selections).some(hasPrice))
  return markets.length === 1 && ['h2h', 'moneyline'].includes(marketKey(markets[0]))
}
...
function OddsButton({ event, market, selection, selected, onToggle }) {
  const label = selectionLabel(selection, market)
  const leg = {
    book: 'Bet105',
    event_id: event.event_id,
    game: eventName(event),
    market_key: marketKey(market),
    market_name: market.market_name || marketKey(market),
    label,
    selection: selection.name || selection.description,
    line: ['h2h', 'moneyline'].includes(marketKey(market)) ? null : selection.line,
    price: selection.price,
    implied_probability: selection?.odds?.implied_probability,
  }
  return <button type="button" style={s.oddsButton(selected)} onClick={() => onToggle(leg)}><div style={s.oddsLabel}>{label}</div><div style={s.oddsPrice}>{american(leg.price)}</div><div style={s.oddsMeta}><span>Bet105</span><span>Imp {pct(leg.implied_probability)}</span></div></button>
}
...
function BetSlip({ legs, stake, setStake, onRemove, onClear }) {
  ...
  const seen = new Set()
  legs.forEach(leg => {
    const key = `${leg.event_id}:${leg.market_key}`
    if (seen.has(key)) warnings.add('Multiple selections from the same game market may be correlated.')
    seen.add(key)
  })
...
function BaseballContext({ event }) {
  return <section style={{ ...s.panel, marginTop: 14 }}><div style={s.panelInner}><div style={s.panelTitle}>MLBGPT Game Context</div><div style={s.panelSub}>Daily Odds-style model context will join here when the selected-game model payload is available. The Bet105 board is not blocked by missing model data.</div><div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(160px,1fr))', gap: 10, marginTop: 12 }}><div style={s.calcBox}><div style={s.calcLabel}>Pitcher Matchup</div><div style={s.calcValue}>Model context unavailable</div></div><div style={s.calcBox}><div style={s.calcLabel}>Top Bats</div><div style={s.calcValue}>Pending data join</div></div><div style={s.calcBox}><div style={s.calcLabel}>Weather / Venue</div><div style={s.calcValue}>Pending data join</div></div><div style={s.calcBox}><div style={s.calcLabel}>Model Edge</div><div style={s.calcValue}>Pending</div></div></div>{event?.coverage_notes?.length > 0 && <div style={{ ...s.warn, marginTop: 12 }}>{event.coverage_notes.join(' ')}</div>}</div></section>
}
...
function MarketBoard({ event, selectedKeys, onToggle }) {
  ...
  return <><section style={s.panel}><div style={s.boardHero}><div style={s.matchup}>{eventName(event)}</div><div style={s.chipRow}><span style={s.chip}>{formatTime(event?.start_time)}</span><span style={s.chip}>Bet105</span><span style={s.chip}>{asArray(event?.markets).length} markets</span>{missingGame && <span style={s.warningChip}>missing team metadata</span>}{onlyMoneyline(event) && <span style={s.warningChip}>Moneyline only from KIBL</span>}</div></div><div style={s.marketWrap}>{ordered.map(category => { const markets = groups[category] || []; if (!markets.length) return null; return <div key={category} style={s.accordion}><div style={s.accordionHead}><div style={s.marketTitle}>{category}</div><span style={s.chip}>{markets.length} markets</span></div>{markets.map((market, idx) => <div key={`${category}-${marketDisplayKey(market)}-${idx}`}><div style={{ ...s.panelSub, padding: '12px 12px 0', color: '#58a6ff', fontWeight: 950 }}>{cleanMarketName(market.market_name || marketKey(market))}</div><div style={s.oddsGrid}>{asArray(market.selections).filter(hasPrice).map((selection, sidx) => { const tempLeg = { event_id: event.event_id, market_key: marketKey(market), label: selectionLabel(selection, market), line: ['h2h', 'moneyline'].includes(marketKey(market)) ? null : selection.line, price: selection.price }; return <OddsButton key={`${selectionLabel(selection, market)}-${selection.price}-${sidx}`} event={event} market={market} selection={selection} selected={selectedKeys.has(legKey(tempLeg))} onToggle={onToggle} /> })}</div></div>)}</div> })}{onlyMoneyline(event) && <div style={s.warn}>Only Moneyline returned by Bet105/KIBL for this fixture request.</div>}</div></section><BaseballContext event={event} /></>
}
...
export default function Bet105SportsbookPage() {
  ...
  function load(forceRefresh = false) {
    ...
    fetchJson(url, { ttlSeconds: TTL, forceRefresh }).then(json => {
      ...
      setLoading(false)
      if (hasPlaceholders(json) || nextEvents.some(onlyMoneyline)) loadDebug(forceRefresh)
    }).catch(err => {
