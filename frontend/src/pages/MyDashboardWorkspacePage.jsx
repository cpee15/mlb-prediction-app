          ...
          return (
            <button
              ...
              onClick={() => {
                ...
                setActiveComponent(component.key)
              }}
              style={activeComponent === component.key ? styles.subNavButtonActive : styles.subNavButton}
            >
...
export default function MyDashboardWorkspacePage() {
  ...
  useEffect(() => {
    ...
    async function bootstrap() {
      try {
        ...
        if (cancelled) return
        setProfile(null)
        setAuthError(err?.name === 'AbortError' ? 'Dashboard bootstrap timed out. Please sign in again.' : 'Unable to load dashboard profile right now.')
      } finally {
  ...
  function setWeight(componentKey, metric, value) {
    setFilters(prev => ({
      ...
      [componentKey]: {
        ...prev[componentKey],
        weights: { ...(prev[componentKey].weights || {}), [metric]: value },
      },
    }))
  }
  ...
  function toggleActiveLineups(componentKey) {
    setActiveLineupsByComponent(prev => ({ ...prev, [componentKey]: !prev[componentKey] }))
  }
  ...
  async function runAllBoards(options = {}) {
    ...
    try {
      const json = await apiJson(`${API}/my-dashboard/solver/batch`, {
        method: 'POST',
    ...
    try {
      await apiJson(`${API}/my-dashboard/items`, {
        ...
        body: JSON.stringify({
          ...
          sort_json: { by: 'score', direction: 'desc', component: componentKey, active_lineups: !!activeLineupsByComponent[componentKey] },
        }),
      })
      await loadWorkspace()
  ...
  async function saveBuilderView() {
    ...
    if (!folderId) {
      setSaveMessage('Choose a folder before saving this builder view.')
      return
    }

    if (!boardResult?.items?.length) {
      setSaveMessage('Run the source board before saving this builder view.')
      return
    }
  ...
  async function createFolder() {
    ...
    try {
      const json = await apiJson(`${API}/my-dashboard/folders`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          folder_name: newFolder.folder_name.trim(),
          folder_date: newFolder.folder_date || null,
          is_default: false,
        }),
      })
