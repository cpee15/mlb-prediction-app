...
OUTPUT_JSON = OUTPUT_DIR / "candidate_bullpen_fatigue_availability_prototype.json"
OUTPUT_CHECKS = OUTPUT_DIR / "candidate_bullpen_fatigue_availability_prototype_checks.csv"
OUTPUT_MATRIX = OUTPUT_DIR / "candidate_bullpen_fatigue_availability_prototype_matrix.csv"
...
def _apply_fatigue(row: Dict[str, Any]) -> Dict[str, Any]:
    ...
    adjusted_quality = _clamp(
        (row.get("role_quality_score") or 0.0) - penalty,
        -0.25,
        0.20,
    )
    adjusted_k = _clamp(
        (row.get("role_k_rate") or 0.0) - penalty * 0.45,
        0.12,
        0.36,
    )
    adjusted_bb = _clamp(
        (row.get("role_bb_rate") or 0.0) + penalty * 0.35,
        0.045,
        0.16,
    )
    adjusted_xwoba = _clamp(
        (row.get("role_xwoba_allowed") or 0.0) + penalty * 0.60,
        0.265,
        0.395,
    )

    if fatigue["availability_status"] == "unavailable":
        notes = "Candidate unavailable due to heavy back-to-back proxy; engine exclusion not yet applied."
    elif fatigue["availability_status"] == "limited":
        notes = "Candidate limited by recent usage proxy; conservative fatigue penalty applied."
    else:
        notes = "Candidate available under deterministic workload proxy."
...
def main() -> None:
    ...
    role_rows_available = len(rows) == expected_rows

    fatigue_fields = [
        "days_rest_proxy",
        "recent_usage_bucket",
        "recent_pitch_count_proxy",
        "back_to_back_risk",
        "availability_multiplier",
        "fatigue_penalty",
    ]
    adjusted_fields = [
        "adjusted_role_quality_score",
        "adjusted_role_k_rate",
        "adjusted_role_bb_rate",
        "adjusted_role_xwoba_allowed",
    ]

    fatigue_fields_present = all(all(row.get(field) is not None for field in fatigue_fields) for row in rows)
    availability_status_present = all(row.get("availability_status") in {"available", "limited", "unavailable"} for row in rows)
    adjusted_rates_present = all(all(row.get(field) is not None for field in adjusted_fields) for row in rows)
    ...
    checks = [
        {
            "check": "role_rows_available",
            "passed": role_rows_available,
            "detail": f"{len(rows)}/{expected_rows}",
        },
        {
            "check": "fatigue_fields_present",
            "passed": fatigue_fields_present,
            "detail": fatigue_fields_present,
        },
        {
            "check": "availability_status_present",
            "passed": availability_status_present,
        ...
        {
            "check": "adjusted_rates_present",
            "passed": adjusted_rates_present,
            "detail": adjusted_rates_present,
        },
        {
    ...
    diagnosis = {
        "diagnosis": "candidate_bullpen_fatigue_availability_prototype_complete",
        ...
        "recommended_next_layer": "6BG_candidate_bullpen_fatigue_availability_analysis",
    }
