...
OUTPUT_JSON = OUTPUT_DIR / "candidate_bullpen_role_selection_shadow.json"
OUTPUT_CHECKS = OUTPUT_DIR / "candidate_bullpen_role_selection_shadow_checks.csv"
OUTPUT_MATRIX = OUTPUT_DIR / "candidate_bullpen_role_selection_shadow_matrix.csv"
OUTPUT_SUMMARY = OUTPUT_DIR / "candidate_bullpen_role_selection_shadow_team_summary.csv"
...
SCENARIOS = [
    {
        "scenario_name": "early_low_leverage",
        "inning": 3,
        "leverage": "low",
        "traffic": "clean",
        "preferred_roles": ["long_relief", "middle_relief"],
        "avoid_roles": ["closer", "setup"],
    },
    {
        "scenario_name": "middle_neutral",
        "inning": 5,
        "leverage": "neutral",
        "traffic": "clean",
        "preferred_roles": ["middle_relief", "long_relief"],
        "avoid_roles": ["closer"],
    },
    {
        "scenario_name": "sixth_high_leverage_traffic",
        "inning": 6,
        "leverage": "high",
        "traffic": "runners_on",
        "preferred_roles": ["high_leverage", "setup"],
        "avoid_roles": ["long_relief"],
    },
    {
        "scenario_name": "seventh_high_leverage",
        "inning": 7,
        "leverage": "high",
        "traffic": "clean",
        "preferred_roles": ["high_leverage", "setup"],
    ...
    {
        "scenario_name": "eighth_setup",
        "inning": 8,
        "leverage": "high",
        "traffic": "clean",
        "preferred_roles": ["setup", "high_leverage"],
        "avoid_roles": ["long_relief"],
    },
    {
        "scenario_name": "ninth_save",
        "inning": 9,
        "leverage": "save",
        "traffic": "clean",
        "preferred_roles": ["closer", "setup"],
        "avoid_roles": ["long_relief", "middle_relief"],
    },
    {
        "scenario_name": "extra_inning_high_leverage",
        "inning": 10,
        "leverage": "high",
        "traffic": "runner_on_second",
        "preferred_roles": ["closer", "setup", "high_leverage"],
        "avoid_roles": ["long_relief"],
    },
    {
        "scenario_name": "blowout_length",
        "inning": 6,
        "leverage": "low",
        "traffic": "length_needed",
        "preferred_roles": ["long_relief"],
        "avoid_roles": ["closer", "setup", "high_leverage"],
    },
]
...
def _select_role(role_rows: List[Dict[str, Any]], scenario: Dict[str, Any]) -> Dict[str, Any]:
    ...
    preferred_set = set(scenario["preferred_roles"])
    selected_direct_match = selected.get("role_name") in preferred_set
    ...
    return {
        ...
        "selected_leverage_bucket": selected.get("leverage_bucket"),
        ...
        "selected_adjusted_k_rate": selected.get("adjusted_role_k_rate"),
        "selected_adjusted_bb_rate": selected.get("adjusted_role_bb_rate"),
        "selected_adjusted_xwoba_allowed": selected.get("adjusted_role_xwoba_allowed"),
...
def main() -> None:
    ...
    _write_csv(OUTPUT_MATRIX, selections)

    summary_rows = []
    team_grouped: Dict[Tuple[Any, Any, Any], List[Dict[str, Any]]] = defaultdict(list)
    for selection in selections:
        team_grouped[(selection["game_pk"], selection["side"], selection["team_id"])].append(selection)

    for key, rows in team_grouped.items():
        ...
        for row in rows:
            ...
            limited_count += 1 if row["selected_availability_status"] == "limited" else 0
            unavailable_count += 1 if row["selected_availability_status"] == "unavailable" else 0
        ...
        summary_rows.append(
            {
                ...
                "limited_selected_count": limited_count,
                "unavailable_selected_count": unavailable_count,
            }
        )

    _write_csv(OUTPUT_SUMMARY, summary_rows)

    expected_rows = 15 * 2 * len(SCENARIOS)
    fatigue_rows_available = len(fatigue_rows) == 15 * 2 * len(ROLE_TEMPLATES)
    scenario_rows_created = len(selections) == expected_rows

    selected_by_team_scenario = len({(r["game_pk"], r["side"], r["team_id"], r["scenario_name"]) for r in selections}) == len(selections)

    unavailable_selected_when_alternative_exists = any(
        row["selected_availability_status"] == "unavailable"
        for row in selections
    )

    no_unavailable_selected_when_alternative_exists = not unavailable_selected_when_alternative_exists

    high_scenarios = {"sixth_high_leverage_traffic", "seventh_high_leverage", "eighth_setup", "ninth_save", "extra_inning_high_leverage"}
    late_roles = {"closer", "setup", "high_leverage"}
    high_leverage_prefers_late_roles = all(
        row["selected_role"] in late_roles
        for row in selections
        if row["scenario_name"] in high_scenarios
    )

    blowout_prefers_long_relief = all(
        row["selected_role"] == "long_relief"
        for row in selections
        if row["scenario_name"] == "blowout_length"
    )

    selection_scores_present = all(row.get("selection_score") is not None for row in selections)
    fallback_reasons_present = all(row.get("fallback_reason") is not None for row in selections)
    ...
    for row in selections:
        ...
        selected_status_counts[str(row["selected_availability_status"])] = selected_status_counts.get(str(row["selected_availability_status"]), 0) + 1
        fallback_count += 0 if row["fallback_reason"] == "none" else 1

    checks = [
        {"check": "fatigue_rows_available", "passed": fatigue_rows_available, "detail": len(fatigue_rows)},
        {"check": "scenario_rows_created", "passed": scenario_rows_created and selected_by_team_scenario, "detail": f"{len(selections)}/{expected_rows}"},
        {"check": "no_unavailable_selected_when_alternative_exists", "passed": no_unavailable_selected_when_alternative_exists, "detail": selected_status_counts},
        {"check": "high_leverage_prefers_late_roles", "passed": high_leverage_prefers_late_roles, "detail": high_leverage_prefers_late_roles},
        {"check": "blowout_prefers_long_relief", "passed": blowout_prefers_long_relief, "detail": blowout_prefers_long_relief},
        {"check": "selection_scores_present", "passed": selection_scores_present, "detail": selection_scores_present},
        {"check": "fallback_reasons_present", "passed": fallback_reasons_present, "detail": fallback_reasons_present},
    ...
    diagnosis = {
        "diagnosis": "candidate_bullpen_role_selection_shadow_complete",
        "fatigue_role_rows": len(fatigue_rows),
        "selection_rows": len(selections),
        "expected_selection_rows": expected_rows,
        ...
        "recommended_next_layer": (
            "6BI_candidate_bullpen_role_selection_shadow_analysis"
