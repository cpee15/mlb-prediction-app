...
OUTPUT_JSON = OUTPUT_DIR / "candidate_bullpen_shadow_diagnostics_export_analysis.json"
OUTPUT_CHECKS = OUTPUT_DIR / "candidate_bullpen_shadow_diagnostics_export_analysis_checks.csv"
OUTPUT_ANALYSIS = OUTPUT_DIR / "candidate_bullpen_shadow_diagnostics_export_analysis_exports.csv"
OUTPUT_FALLBACK = OUTPUT_DIR / "candidate_bullpen_shadow_diagnostics_export_analysis_fallback_event_summary.csv"
OUTPUT_DEPLETION = OUTPUT_DIR / "candidate_bullpen_shadow_diagnostics_export_analysis_depletion_step_summary.csv"
OUTPUT_EMERGENCY = OUTPUT_DIR / "candidate_bullpen_shadow_diagnostics_export_analysis_emergency_summary.csv"
...
REQUIRED_CURVE_FIELDS = ["step", "event", "depletion_index"]
REQUIRED_FALLBACK_FIELDS = ["step", "event", "selected_role", "fallback_reason", "depletion_index"]
REQUIRED_EMERGENCY_FIELDS = ["selected_role", "pre_status", "depletion_index", "fallback_reason"]
...
def main() -> None:
    ...
    for export in exports:
        ...
        curve_values = [float(point["depletion_index"]) for point in export["depletion_curve"]]
        ...
        analysis_rows.append({
            ...
            "timeline_rows": len(export["role_usage_timeline"]),
            "depletion_points": len(export["depletion_curve"]),
            ...
            "curve_non_decreasing": _is_non_decreasing(curve_values),
        ...
        for row in export["role_usage_timeline"]:
            all_timeline.append({"game_pk": export["game_pk"], "side": export["side"], **row})

        for row in export["depletion_curve"]:
            all_depletion.append({"game_pk": export["game_pk"], "side": export["side"], **row})

        for row in export["fallback_events"]:
            all_fallbacks.append({"game_pk": export["game_pk"], "side": export["side"], **row})
        ...
        emergency_rows.append({
            ...
            "team_id": export["team_id"],
            "team_name": export["team_name"],
            **export["emergency_state"],
        })

    _write_csv(OUTPUT_ANALYSIS, analysis_rows)

    fallback_summary_rows = []
    for event_name in [event["event_name"] for event in SEQUENCE_EVENTS]:
        rows = [row for row in all_fallbacks if row["event"] == event_name]
        fallback_summary_rows.append({
            ...
            "avg_depletion_index": _mean([float(row["depletion_index"]) for row in rows]) if rows else None,
        })
    _write_csv(OUTPUT_FALLBACK, fallback_summary_rows)

    depletion_summary_rows = []
    for step in range(1, 9):
        rows = [row for row in all_depletion if int(row["step"]) == step]
        depletion_summary_rows.append({
            "step": step,
            "event": rows[0]["event"] if rows else None,
            "point_count": len(rows),
            "avg_depletion_index": _mean([float(row["depletion_index"]) for row in rows]),
            "min_depletion_index": round(min(float(row["depletion_index"]) for row in rows), 4) if rows else None,
            "max_depletion_index": round(max(float(row["depletion_index"]) for row in rows), 4) if rows else None,
        })
    _write_csv(OUTPUT_DEPLETION, depletion_summary_rows)

    emergency_summary_rows = []
    for role, count in _count(emergency_rows, "selected_role").items():
        rows = [row for row in emergency_rows if row["selected_role"] == role]
        emergency_summary_rows.append({
            "emergency_selected_role": role,
            "count": count,
            "pre_status_counts": json.dumps(_count(rows, "pre_status"), sort_keys=True),
            "avg_depletion_index": _mean([float(row["depletion_index"]) for row in rows]),
        })
    _write_csv(OUTPUT_EMERGENCY, emergency_summary_rows)

    diagnostics_exports_available = (
        len(exports) == 30
        and len(all_timeline) == 240
        and len(all_depletion) == 240
        and len(all_fallbacks) == sum(int(row["fallback_count"]) for row in analysis_rows)
    )

    schema_consistency_valid = all(
        all(field in export for field in REQUIRED_EXPORT_FIELDS)
        and all(field in export["summary"] for field in REQUIRED_SUMMARY_FIELDS)
        and all(all(field in row for field in REQUIRED_TIMELINE_FIELDS) for row in export["role_usage_timeline"])
        and all(all(field in row for field in REQUIRED_CURVE_FIELDS) for row in export["depletion_curve"])
        and all(all(field in row for field in REQUIRED_FALLBACK_FIELDS) for row in export["fallback_events"])
        and all(field in export["emergency_state"] for field in REQUIRED_EMERGENCY_FIELDS)
        for export in exports
    ) and len(set(export["diagnostics_version"] for export in exports)) == 1 and len(set(export["contract_version"] for export in exports)) == 1

    diagnostics_bounds_valid = all(
        1.0 <= float(export["summary"]["final_depletion_index"]) <= 1.5
        and 1.0 <= float(export["summary"]["max_depletion_index"]) <= 1.5
        and 0.0 <= float(export["summary"]["fallback_rate"]) <= 0.5
        and 0 <= int(export["summary"]["exhausted_role_count_final"]) <= 8
        and all(_safe_float(point["depletion_index"]) is not None for point in export["depletion_curve"])
        and _is_non_decreasing([float(point["depletion_index"]) for point in export["depletion_curve"]])
        for export in exports
    )

    diagnostics_usability_valid = all(
        bool(export["summary"].get("dominant_selected_role"))
        and bool(export["summary"].get("emergency_selected_role"))
        and all(key in export["availability_snapshot"] for key in ["initial_status_counts", "selected_pre_status_counts", "selected_role_counts"])
        and all(bool(event.get("fallback_reason")) for event in export["fallback_events"])
        and {"production_engine", "sportsbook_outputs", "frontend_ui"}.issubset(set(export["blocked_consumers"]))
        for export in exports
    )
    ...
    safety_boundaries_valid = all(
        ...
        and all(export["safety_flags"].values())
        for export in exports
    )

    aggregate_metrics = {
        "diagnostics_export_count": len(exports),
        "total_timeline_rows": len(all_timeline),
        "total_depletion_points": len(all_depletion),
        "total_fallback_events": len(all_fallbacks),
        "average_fallback_count": _mean([float(row["fallback_count"]) for row in analysis_rows]),
        "max_fallback_count": max(int(row["fallback_count"]) for row in analysis_rows),
        "average_final_depletion_index": _mean([float(row["final_depletion_index"]) for row in analysis_rows]),
        "max_final_depletion_index": round(max(float(row["final_depletion_index"]) for row in analysis_rows), 4),
        "emergency_role_counts": _count(emergency_rows, "selected_role"),
        "dominant_role_counts": _count(analysis_rows, "dominant_selected_role"),
        "fallback_count_by_event": {row["event"]: row["fallback_count"] for row in fallback_summary_rows},
        "average_depletion_by_step": {str(row["step"]): row["avg_depletion_index"] for row in depletion_summary_rows},
    }

    aggregate_metrics_valid = (
        aggregate_metrics["diagnostics_export_count"] == 30
        and aggregate_metrics["total_timeline_rows"] == 240
        and aggregate_metrics["total_depletion_points"] == 240
        and aggregate_metrics["total_fallback_events"] == 32
        and aggregate_metrics["max_final_depletion_index"] <= 1.5
        and all(value is not None for value in aggregate_metrics["average_depletion_by_step"].values())
    )

    candidate_mode_only = safety_boundaries_valid

    checks = [
        {"check": "diagnostics_exports_available", "passed": diagnostics_exports_available, "detail": f"{len(exports)} exports / {len(all_timeline)} timeline / {len(all_depletion)} curve / {len(all_fallbacks)} fallbacks"},
        {"check": "schema_consistency_valid", "passed": schema_consistency_valid, "detail": schema_consistency_valid},
        {"check": "diagnostics_bounds_valid", "passed": diagnostics_bounds_valid, "detail": diagnostics_bounds_valid},
        {"check": "diagnostics_usability_valid", "passed": diagnostics_usability_valid, "detail": diagnostics_usability_valid},
        {"check": "safety_boundaries_valid", "passed": safety_boundaries_valid, "detail": safety_boundaries_valid},
        {"check": "aggregate_metrics_valid", "passed": aggregate_metrics_valid, "detail": aggregate_metrics},
    ...
    diagnosis = {
        "diagnosis": "candidate_bullpen_shadow_diagnostics_export_analysis_complete",
        "diagnostics_version": DIAGNOSTICS_VERSION,
        "contract_version": CONTRACT_VERSION,
        ...
        "recommended_next_layer": (
            "6BS_candidate_bullpen_shadow_calibration_gate_audit"
