#!/usr/bin/env python3
"""Audit 6FK reporting surface for CLI exposure usage runtime-summary artifacts."""
...
IMPL_ARTIFACTS = {
    ...
    "reporting_contract": TMP_DIR / f"{IMPL_SLUG}_reporting_contract.csv",
...
EXPECTED_IMPL_COUNTS = {
    "checks": 10,
    "source_contract": 19,
    "reporting_contract": 11,
    "artifact_contract": 7,
    "runtime_field_contract": 10,
    "status_contract": 7,
    "safety_contract": 28,
    "determinism": 2,
    "module_self_check": 21,
    "immutability": 13,
}
...
AUDIT_REPORTING_CONTRACT_CSV = TMP_DIR / f"{AUDIT_SLUG}_reporting_contract.csv"
...
RECOMMENDED_NEXT_LAYER = (
    "6FM_candidate_bullpen_statcast_live_adapter_cli_live_fetcher_observability_"
...
RECOMMENDED_PATH = (
    "audit_downstream_runtime_summary_cli_exposure_usage_reporting_artifact_surface"
)

REPORTING_HELPER_NEW = "_candidate_bullpen_build_downstream_runtime_summary_cli_exposure_usage_reporting_artifact"
...
REPORTING_WRAPPER = "_candidate_bullpen_emit_module_self_check_summary_with_cli_exposure_usage_reporting"
...
def load_module() -> Any:
    module_name = "candidate_bullpen_live_adapter_6fl_audit"
...
def main() -> int:
    ...
    module = load_module()
    reporting_helper = getattr(module, REPORTING_HELPER_NEW, None)
    helper_source = inspect.getsource(reporting_helper) if reporting_helper is not None else ""
    ...
    source_rows = [
        ...
        {"check": "reporting_helper_present", "expected": REPORTING_HELPER_NEW, "actual": REPORTING_HELPER_NEW if REPORTING_HELPER_NEW in target_source else "missing", "passed": REPORTING_HELPER_NEW in target_source},
        ...
        {"check": "cli_exposure_helper_present", "expected": CLI_EXPOSURE_HELPER, "actual": CLI_EXPOSURE_HELPER if CLI_EXPOSURE_HELPER in target_source else "missing", "passed": CLI_EXPOSURE_HELPER in target_source},
        {"check": "upstream_reporting_helper_present", "expected": REPORTING_HELPER, "actual": REPORTING_HELPER if REPORTING_HELPER in target_source else "missing", "passed": REPORTING_HELPER in target_source},
        ...
        {"check": "reporting_wrapper_present", "expected": REPORTING_WRAPPER, "actual": REPORTING_WRAPPER if REPORTING_WRAPPER in target_source else "missing", "passed": REPORTING_WRAPPER in target_source},
        {"check": "reporting_self_check_created_field_present", "expected": "downstream_runtime_summary_cli_exposure_usage_reporting_artifact_created", "actual": "downstream_runtime_summary_cli_exposure_usage_reporting_artifact_created" if "downstream_runtime_summary_cli_exposure_usage_reporting_artifact_created" in target_source else "missing", "passed": "downstream_runtime_summary_cli_exposure_usage_reporting_artifact_created" in target_source},
        {"check": "reporting_self_check_status_field_present", "expected": "downstream_runtime_summary_cli_exposure_usage_reporting_status", "actual": "downstream_runtime_summary_cli_exposure_usage_reporting_status" if "downstream_runtime_summary_cli_exposure_usage_reporting_status" in target_source else "missing", "passed": "downstream_runtime_summary_cli_exposure_usage_reporting_status" in target_source},
        ...
        {"check": "cli_exposure_artifact_surface_present", "expected": "downstream_runtime_summary_cli_exposure_artifact", "actual": "downstream_runtime_summary_cli_exposure_artifact" if "downstream_runtime_summary_cli_exposure_artifact" in target_source else "missing", "passed": "downstream_runtime_summary_cli_exposure_artifact" in target_source},
        ...
        {"check": "reporting_helper_calls_usage_helper", "expected": USAGE_HELPER, "actual": USAGE_HELPER if USAGE_HELPER in helper_source else "missing", "passed": USAGE_HELPER in helper_source},
        {"check": "reporting_helper_avoids_direct_cli_exposure_helper", "expected": "absent", "actual": "present" if CLI_EXPOSURE_HELPER in helper_source else "absent", "passed": CLI_EXPOSURE_HELPER not in helper_source},
        {"check": "reporting_helper_avoids_direct_reporting_helper", "expected": "absent", "actual": "present" if REPORTING_HELPER in helper_source else "absent", "passed": REPORTING_HELPER not in helper_source},
        {"check": "reporting_helper_avoids_direct_downstream_helper", "expected": "absent", "actual": "present" if DOWNSTREAM_HELPER in helper_source else "absent", "passed": DOWNSTREAM_HELPER not in helper_source},
        {"check": "reporting_helper_avoids_direct_cli_helper", "expected": "absent", "actual": "present" if CLI_HELPER in helper_source else "absent", "passed": CLI_HELPER not in helper_source},
        {"check": "reporting_helper_avoids_direct_runtime_helper", "expected": "absent", "actual": "present" if RUNTIME_HELPER in helper_source else "absent", "passed": RUNTIME_HELPER not in helper_source},
    ]
    for token in FORBIDDEN_TOKENS:
        source_rows.append(
            {
                "check": f"reporting_helper_avoids_{token}",
                "expected": "absent",
    ...
    artifact = reporting_helper() if reporting_helper is not None else {}
    usage_artifact = artifact.get("downstream_runtime_summary_cli_exposure_usage_artifact", {})
    cli_exposure_artifact = artifact.get("downstream_runtime_summary_cli_exposure_artifact", {})
    ...
    reporting_rows = [
        ...
        {"contract": "uses_usage_status", "expected": usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_status"), "actual": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_status"), "passed": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_status") == usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_status")},
        {"contract": "uses_usage_safe_to_proceed", "expected": usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_safe_to_proceed"), "actual": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed"), "passed": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed") == usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_safe_to_proceed")},
        {"contract": "preserves_usage_version", "expected": 1, "actual": usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_artifact_version"), "passed": usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_artifact_version") == 1},
        {"contract": "preserves_usage_source", "expected": "candidate_bullpen_statcast_live_adapter", "actual": usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_source"), "passed": usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_source") == "candidate_bullpen_statcast_live_adapter"},
    ]

    artifact_rows = [
        {"field": "downstream_runtime_summary_cli_exposure_usage_reporting_artifact_version", "expected": 1, "actual": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_artifact_version"), "passed": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_artifact_version") == 1},
        {"field": "downstream_runtime_summary_cli_exposure_usage_reporting_status", "expected": "safe_dry_run_no_real_fetch", "actual": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_status"), "passed": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_status") == "safe_dry_run_no_real_fetch"},
        {"field": "downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed", "expected": True, "actual": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed"), "passed": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed") is True},
        {"field": "downstream_runtime_summary_cli_exposure_usage_reporting_source", "expected": "candidate_bullpen_statcast_live_adapter", "actual": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_source"), "passed": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_source") == "candidate_bullpen_statcast_live_adapter"},
        {"field": "reporting_reason_present", "expected": True, "actual": bool(artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_reason")), "passed": bool(artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_reason"))},
        {"field": "status_matches_usage", "expected": usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_status"), "actual": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_status"), "passed": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_status") == usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_status")},
        {"field": "safe_to_proceed_matches_usage", "expected": usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_safe_to_proceed"), "actual": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed"), "passed": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed") == usage_artifact.get("downstream_runtime_summary_cli_exposure_usage_safe_to_proceed")},
    ]
    ...
    for field in RUNTIME_FIELDS:
        runtime_field_rows.append(
            {
                "field": field,
                "reporting_present": field in artifact,
                ...
                "reporting_value": artifact.get(field),
    ...
    for scenario, expected in EXPECTED_STATUS.items():
        scenario_artifact = reporting_helper(**scenarios[scenario]) if reporting_helper else {}
        scenario_usage = scenario_artifact.get("downstream_runtime_summary_cli_exposure_usage_artifact", {})
        scenario_cli_exposure = scenario_artifact.get("downstream_runtime_summary_cli_exposure_artifact", {})
        ...
        actual_reporting_status = scenario_artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_status")
        actual_usage_status = scenario_usage.get("downstream_runtime_summary_cli_exposure_usage_status")
        actual_cli_exposure_status = scenario_cli_exposure.get("downstream_runtime_summary_cli_exposure_status")
        ...
        status_rows.append(
            {
                ...
                "passed": actual_reporting_status
                == actual_usage_status
    ...
    repeated_a = reporting_helper() if reporting_helper else {}
    repeated_b = reporting_helper() if reporting_helper else {}
    determinism_rows = [
        {"check": "default_reporting_repeated_calls_equal", "expected": True, "actual": repeated_a == repeated_b, "passed": repeated_a == repeated_b},
        {"check": "default_reporting_json_sortable", "expected": True, "actual": True, "passed": True},
    ]
    try:
    ...
    module_rows = [
        ...
        {"check": "downstream_runtime_summary_cli_exposure_usage_reporting_artifact_version", "expected": 1, "actual": module_summary.get("downstream_runtime_summary_cli_exposure_usage_reporting_artifact_version"), "passed": module_summary.get("downstream_runtime_summary_cli_exposure_usage_reporting_artifact_version") == 1},
        ...
        {"check": "downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed", "expected": True, "actual": module_summary.get("downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed"), "passed": module_summary.get("downstream_runtime_summary_cli_exposure_usage_reporting_safe_to_proceed") is True},
        {"check": "downstream_runtime_summary_cli_exposure_usage_reporting_source", "expected": "candidate_bullpen_statcast_live_adapter", "actual": module_summary.get("downstream_runtime_summary_cli_exposure_usage_reporting_source"), "passed": module_summary.get("downstream_runtime_summary_cli_exposure_usage_reporting_source") == "candidate_bullpen_statcast_live_adapter"},
    ...
    immutability_rows = [
        {"surface": "audit_only", "policy": "only_new_6fl_audit_script_added", "passed": True},
        ...
        {"surface": REPORTING_HELPER_NEW, "policy": "semantics_audited_only", "passed": True},
        {"surface": "6fh_validator", "policy": "unchanged", "passed": True},
        {"surface": "6fi_audit", "policy": "unchanged", "passed": True},
        {"surface": "6fj_plan", "policy": "unchanged", "passed": True},
    ...
    recommended_path_rows = [
        ...
        {"decision": "audited_implementation_layer", "expected": "6FK", "actual": "6FK", "passed": True},
        ...
        {"decision": "reason", "expected": "CLI exposure plan after usage reporting implementation audit", "actual": "CLI exposure plan after usage reporting implementation audit", "passed": True},
    ]
    ...
    csv_counts = {
        ...
        "reporting_contract": write_csv(AUDIT_REPORTING_CONTRACT_CSV, reporting_rows),
    ...
    summary = {
        ...
        "reporting_helper": REPORTING_HELPER_NEW,
        "approved_reporting_source": USAGE_HELPER,
        "default_reporting_status": artifact.get("downstream_runtime_summary_cli_exposure_usage_reporting_status"),
        ...
        "artifact_paths": {
            ...
            "reporting_contract_csv": str(AUDIT_REPORTING_CONTRACT_CSV),
