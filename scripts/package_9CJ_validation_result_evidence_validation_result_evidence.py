...
Materializes deterministic result-evidence records over Layer 9CH
validation-result evidence validation records according to the Layer 9CI
...
OUTPUT_DIR = ROOT / "tmp" / "layer_9CJ_validation_result_evidence"
...
def replay_plan() -> dict[str, Any]:
    plan = load_module(PLAN_PATH, "layer_9ci_plan")
    ...
    if (
        ...
        raise RuntimeError(
            "Unexpected Layer 9CI plan version: "
...
def build_result_evidence_records(
    ...
    for source in source_records:
        ...
        record = {
            ...
            "validation_result_evidence_validation_source_record_"
            "identity_digest":
                source[
                    "validation_result_evidence_validation_"
                    "plan_identity_digest"
            ...
            "validation_result_evidence_validation_result_evidence_"
            "rationale": (
                "The Layer 9CH validation record was replayed "
            ...
            "authority_boundary": (
                ...
                "the Layer 9CH validation record. It grants no authority "
...
def main() -> int:
    ...
    implementation_authorities = sum(
        bool(
            row[
                "validation_result_evidence_validation_"
                "result_evidence_implementation_authority_granted"
            ]
        )
        for row in result_records
    )
    ...
    checks = [
        ...
        },
        {
            ...
            "expected": EXPECTED_LAYER_9CG_PLAN_DIGEST,
            "passed": (
                replay["plan_digest"]
                == EXPECTED_LAYER_9CG_PLAN_DIGEST
            ),
        ...
        {
            "check": "nine_cf_result_digest_preserved",
            "actual": replay["source_result_digest"],
            "expected": EXPECTED_LAYER_9CF_RESULT_DIGEST,
            "passed": (
            ...
            "actual": len(
                {
                    row[
                        "validation_result_evidence_validation_"
                        "result_evidence_plan_record_id"
                    ]
                    for row in result_records
                }
            ),
