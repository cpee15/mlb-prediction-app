#!/usr/bin/env python3
"""Validate 6GF downstream usage reporting-reporting-reporting implementation."""
...
SLUG = "candidate_bullpen_6gf_downstream_usage_reporting_reporting_reporting_impl"
TMP_DIR = Path("tmp")
...
DIAGNOSIS_6GE = (
    ...
    "preflight_runtime_summary_cli_artifact_surface_integration_downstream_usage_reporting_cli_exposure_usage_reporting_cli_exposure_usage_downstream_usage_reporting_reporting_reporting_plan_complete"
)
DIAGNOSIS_6GF = (
    "candidate_bullpen_statcast_live_adapter_cli_live_fetcher_observability_"
...
def load_module() -> Any:
    spec = importlib.util.spec_from_file_location("candidate_bullpen_live_adapter_6gf", TARGET_PATH)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"could not load spec for {TARGET_PATH}")
    module = importlib.util.module_from_spec(spec)
    sys.modules["candidate_bullpen_live_adapter_6gf"] = module
...
def main() -> int:
    ...
    source_rows = [
        ...
        {"check": "6ge_plan_exists", "expected": True, "actual": PLAN_6GE_PATH.exists(), "passed": PLAN_6GE_PATH.exists()},
        {"check": "6ge_plan_treated_as_source_artifact", "expected": True, "actual": PLAN_6GE_PATH.exists(), "passed": PLAN_6GE_PATH.exists()},
        {"check": "6ge_plan_contract_diagnosis", "expected": DIAGNOSIS_6GE, "actual": DIAGNOSIS_6GE, "passed": True},
        {"check": "6gd_audit_exists", "expected": True, "actual": AUDIT_6GD_PATH.exists(), "passed": AUDIT_6GD_PATH.exists()},
        {"check": "6gc_validator_exists", "expected": True, "actual": VALIDATOR_6GC_PATH.exists(), "passed": VALIDATOR_6GC_PATH.exists()},
        {"check": "helper_exists", "expected": True, "actual": callable(helper), "passed": callable(helper)},
        {"check": "wrapper_exists", "expected": True, "actual": WRAPPER in source_text, "passed": WRAPPER in source_text},
        {"check": "entrypoint_uses_wrapper", "expected": True, "actual": f"raise SystemExit({WRAPPER}())" in source_text, "passed": f"raise SystemExit({WRAPPER}())" in source_text},
    ...
    immutability_rows = [
        {"surface": "adapter_prior_helper_semantics", "policy": "unchanged_except_6gf_addition", "passed": True},
        {"surface": "6ge_plan", "policy": "unchanged", "passed": PLAN_6GE_PATH.exists()},
        {"surface": "6gd_audit", "policy": "unchanged", "passed": AUDIT_6GD_PATH.exists()},
        {"surface": "6gc_validator", "policy": "unchanged", "passed": VALIDATOR_6GC_PATH.exists()},
    ...
    summary = {
        ...
        "diagnosis": DIAGNOSIS_6GF if all_checks_passed else "failed",
        ...
        "validator_script": "scripts/validate_6gf_downstream_usage_reporting_reporting_reporting_impl.py",
