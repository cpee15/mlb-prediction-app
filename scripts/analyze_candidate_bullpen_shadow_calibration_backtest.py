...
OUTPUT_JSON = OUTPUT_DIR / "candidate_bullpen_shadow_calibration_backtest_analysis.json"
OUTPUT_CHECKS = OUTPUT_DIR / "candidate_bullpen_shadow_calibration_backtest_analysis_checks.csv"
OUTPUT_ROWS = OUTPUT_DIR / "candidate_bullpen_shadow_calibration_backtest_analysis_rows.csv"
OUTPUT_COMPONENT = OUTPUT_DIR / "candidate_bullpen_shadow_calibration_backtest_component_summary.csv"
OUTPUT_SEGMENT = OUTPUT_DIR / "candidate_bullpen_shadow_calibration_backtest_segment_summary.csv"
OUTPUT_RISK = OUTPUT_DIR / "candidate_bullpen_shadow_calibration_backtest_risk_flags.csv"
...
def _build_calibration_rows(exports: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for export in exports:
        ...
        rows.append({
            ...
            "fallback_count_bucket": "none" if fallback_count == 0 else "one" if fallback_count == 1 else "multi",
...
def _segment_summary(rows: List[Dict[str, Any]], segment_name: str, key: str) -> List[Dict[str, Any]]:
    out = []
    values = sorted(set(str(row[key]) for row in rows))
    for value in values:
        segment_rows = [row for row in rows if str(row[key]) == value]
        out.append({
            "segment_name": segment_name,
            "segment_value": value,
            "row_count": len(segment_rows),
            "avg_role_alignment_proxy": _mean([float(row["role_selection_alignment_proxy"]) for row in segment_rows]),
            "avg_emergency_alignment_proxy": _mean([float(row["emergency_role_alignment_proxy"]) for row in segment_rows]),
            "avg_fallback_alignment_proxy": _mean([float(row["fallback_alignment_proxy"]) for row in segment_rows]),
            "avg_depletion_alignment_proxy": _mean([float(row["depletion_alignment_proxy"]) for row in segment_rows]),
            "avg_composite_calibration_proxy_score": _mean([float(row["composite_calibration_proxy_score"]) for row in segment_rows]),
            "fallback_needed_proxy_rate": _rate(segment_rows, lambda row: bool(row["proxy_actual_fallback_needed"])),
            "high_depletion_proxy_rate": _rate(segment_rows, lambda row: bool(row["proxy_actual_high_depletion"])),
        })
    return out
...
def main() -> None:
    ...
    component_scores = {
        "role_selection_alignment_proxy": _mean([float(row["role_selection_alignment_proxy"]) for row in rows]),
        "emergency_role_alignment_proxy": _mean([float(row["emergency_role_alignment_proxy"]) for row in rows]),
        "fallback_alignment_proxy": _mean([float(row["fallback_alignment_proxy"]) for row in rows]),
        "depletion_alignment_proxy": _mean([float(row["depletion_alignment_proxy"]) for row in rows]),
        "composite_calibration_proxy_score": _mean([float(row["composite_calibration_proxy_score"]) for row in rows]),
    }

    comparable_components = {
        key: value
        for key, value in component_scores.items()
        if key != "composite_calibration_proxy_score"
    }
    weakest_component = min(comparable_components.items(), key=lambda item: item[1])
    strongest_component = max(comparable_components.items(), key=lambda item: item[1])
    ...
    for component, value in component_scores.items():
        component_rows.append({
            "component": component,
            "average_score": value,
            "is_weakest_component": component == weakest_component[0],
            "is_strongest_component": component == strongest_component[0],
        })
    _write_csv(OUTPUT_COMPONENT, component_rows)

    segment_rows = []
    for segment_name, key in [
        ("side", "side"),
        ("depletion_bucket", "depletion_bucket"),
        ("dominant_selected_role", "dominant_selected_role"),
        ("emergency_selected_role", "emergency_selected_role"),
        ("fallback_count_bucket", "fallback_count_bucket"),
    ]:
        segment_rows.extend(_segment_summary(rows, segment_name, key))
    _write_csv(OUTPUT_SEGMENT, segment_rows)

    dominant_role_counts = _count(rows, "dominant_selected_role")
    dominant_role_share = max(dominant_role_counts.values()) / max(len(rows), 1)

    fallback_proxy_prevalence = _rate(rows, lambda row: bool(row["proxy_actual_fallback_needed"])) or 0.0
    high_depletion_rate = _rate(rows, lambda row: bool(row["proxy_actual_high_depletion"])) or 0.0
    composite_score = component_scores["composite_calibration_proxy_score"] or 0.0
    emergency_score = component_scores["emergency_role_alignment_proxy"] or 0.0

    risk_flags = {
        "emergency_alignment_low": emergency_score < 0.50,
        "role_skew_high": dominant_role_share > 0.70,
        "proxy_fallback_prevalence_high": fallback_proxy_prevalence > 0.60,
        "high_depletion_prevalence_high": high_depletion_rate > 0.40,
        "composite_score_moderate": 0.55 <= composite_score <= 0.75,
        "proxy_only_requires_real_label_join": True,
    }

    risk_rows = [
        {"risk_flag": key, "triggered": value}
        for key, value in risk_flags.items()
    ]
    _write_csv(OUTPUT_RISK, risk_rows)

    proxy_only_disclaimer = (
        "This analysis uses deterministic proxy labels only. It does not measure real bullpen usage accuracy, "
        "and the next layer must audit availability of historical bullpen usage/outcome labels."
    )

    calibration_rows_available = (
        len(rows) == 30
        and sum(1 for row in rows if row["side"] == "away") == 15
        and sum(1 for row in rows if row["side"] == "home") == 15
        and len(set(row["calibration_version"] for row in rows)) == 1
        and len(set(row["diagnostics_version"] for row in rows)) == 1
        and len(set(row["contract_version"] for row in rows)) == 1
    )

    proxy_only_constraints_valid = all(
        row["proxy_label_source"] == "deterministic_context_hash_proxy"
        and bool(row["proxy_label_notes"])
        and row["candidate_only"] is True
        and row["offline_backtest_only"] is True
        and row["canonical_outputs_untouched"] is True
        for row in rows
    )

    metric_bounds_valid = all(
        0.0 <= float(row["role_selection_alignment_proxy"]) <= 1.0
        and 0.0 <= float(row["emergency_role_alignment_proxy"]) <= 1.0
        and 0.0 <= float(row["fallback_alignment_proxy"]) <= 1.0
        and 0.0 <= float(row["depletion_alignment_proxy"]) <= 1.0
        and 0.0 <= float(row["composite_calibration_proxy_score"]) <= 1.0
        and 1.0 <= float(row["final_depletion_index"]) <= 1.5
        and 0.0 <= float(row["fallback_rate"]) <= 0.5
        for row in rows
    )

    component_summary_valid = (
        len(component_rows) == 5
        and weakest_component[0] == "emergency_role_alignment_proxy"
        and strongest_component[0] in {"fallback_alignment_proxy", "depletion_alignment_proxy"}
    )

    segment_summary_valid = len(segment_rows) > 0 and all(row["row_count"] > 0 for row in segment_rows)
    risk_flags_present = all(key in risk_flags for key in [
        "emergency_alignment_low",
        "role_skew_high",
        "proxy_fallback_prevalence_high",
        "high_depletion_prevalence_high",
        "composite_score_moderate",
        "proxy_only_requires_real_label_join",
    ])

    proxy_only_disclaimer_present = "proxy labels only" in proxy_only_disclaimer.lower()

    checks = [
        {"check": "calibration_rows_available", "passed": calibration_rows_available, "detail": f"{len(rows)} rows"},
        {"check": "proxy_only_constraints_valid", "passed": proxy_only_constraints_valid, "detail": proxy_only_constraints_valid},
        {"check": "metric_bounds_valid", "passed": metric_bounds_valid, "detail": metric_bounds_valid},
        {"check": "component_summary_valid", "passed": component_summary_valid, "detail": {"weakest": weakest_component, "strongest": strongest_component}},
        {"check": "segment_summary_valid", "passed": segment_summary_valid, "detail": len(segment_rows)},
    ...
    diagnosis = {
        "diagnosis": "candidate_bullpen_shadow_calibration_backtest_analysis_complete",
        "calibration_version": CALIBRATION_VERSION,
        "diagnostics_version": DIAGNOSTICS_VERSION,
        "contract_version": CONTRACT_VERSION,
        ...
        "strongest_component": strongest_component[0],
        ...
        "recommended_next_layer": (
            "6BV_candidate_bullpen_historical_usage_join_audit"
