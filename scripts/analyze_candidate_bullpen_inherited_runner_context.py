...
OUTPUT_JSON = OUTPUT_DIR / "candidate_bullpen_inherited_runner_context_analysis.json"
OUTPUT_CHECKS = OUTPUT_DIR / "candidate_bullpen_inherited_runner_context_analysis_checks.csv"
OUTPUT_ROWS = OUTPUT_DIR / "candidate_bullpen_inherited_runner_context_analysis_rows.csv"
OUTPUT_SCENARIO = OUTPUT_DIR / "candidate_bullpen_inherited_runner_context_analysis_scenario_summary.csv"
OUTPUT_TEAM = OUTPUT_DIR / "candidate_bullpen_inherited_runner_context_analysis_team_summary.csv"
...
def _inherited_adjustments(selected: Dict[str, Any], scenario: Dict[str, Any]) -> Dict[str, Any]:
    ...
    exposure_penalty = {"low": 0.000, "medium": 0.006, "high": 0.010}.get(str(selected.get("inherited_runner_exposure")), 0.006)
    ...
    strand_support_score = _clamp(k_support + csw_support - bb_risk - exposure_penalty, -0.10, 0.20) or 0.0
    ...
    return {
        ...
        "adjusted_inherited_runner_quality_score": _clamp(float(selected.get("adjusted_role_quality_score") or 0.0) + quality_boost - traffic_penalty, -0.30, 0.22),
        "adjusted_inherited_runner_k_rate": _clamp(float(selected.get("adjusted_role_k_rate") or 0.0) + strand_support_score * 0.10, 0.12, 0.38),
        "adjusted_inherited_runner_bb_rate": _clamp(float(selected.get("adjusted_role_bb_rate") or 0.0) + strand_pressure_index * 0.006, 0.045, 0.17),
        "adjusted_inherited_runner_xwoba_allowed": _clamp(float(selected.get("adjusted_role_xwoba_allowed") or 0.0) + strand_pressure_index * 0.012 - strand_support_score * 0.018, 0.265, 0.410),
    }


def _build_context_rows() -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
...
def main() -> None:
    fatigue_rows, rows = _build_context_rows()
    _write_csv(OUTPUT_ROWS, rows)

    grouped_scenario: Dict[str, List[Dict[str, Any]]] = defaultdict(list)
    for row in rows:
        grouped_scenario[str(row["scenario_name"])].append(row)

    scenario_rows = []
    for scenario_name, scenario_rows_raw in sorted(grouped_scenario.items()):
        scenario_rows.append({
            "scenario_name": scenario_name,
            "row_count": len(scenario_rows_raw),
            "role_counts": json.dumps(_count(scenario_rows_raw, "selected_role"), sort_keys=True),
            "avg_pressure": _mean([float(r["strand_pressure_index"]) for r in scenario_rows_raw]),
            "avg_risk": _mean([float(r["inherited_runner_risk_score"]) for r in scenario_rows_raw]),
            "avg_strand_support": _mean([float(r["strand_support_score"]) for r in scenario_rows_raw]),
            "avg_adjusted_quality": _mean([float(r["adjusted_inherited_runner_quality_score"]) for r in scenario_rows_raw]),
        })
    _write_csv(OUTPUT_SCENARIO, scenario_rows)
    ...
    for key, team_contexts in grouped_team.items():
        ...
        team_rows.append({
            ...
            "role_counts": json.dumps(_count(team_contexts, "selected_role"), sort_keys=True),
            "avg_pressure": _mean([float(r["strand_pressure_index"]) for r in team_contexts]),
            "avg_risk": _mean([float(r["inherited_runner_risk_score"]) for r in team_contexts]),
            "avg_support": _mean([float(r["strand_support_score"]) for r in team_contexts]),
        })
    _write_csv(OUTPUT_TEAM, team_rows)

    expected_fatigue_rows = 15 * 2 * len(ROLE_TEMPLATES)
    expected_context_rows = 15 * 2 * len(INHERITED_SCENARIOS)
    expected_team_sides = 15 * 2

    by_pressure = {row["scenario_name"]: float(row["avg_pressure"]) for row in scenario_rows}
    by_risk = {row["scenario_name"]: float(row["avg_risk"]) for row in scenario_rows}

    pressure_ordering_valid = (
        by_pressure["clean_start"] == 0.0
        and by_pressure["runner_on_first_one_out"] < by_pressure["runner_on_second_no_out"]
        and by_pressure["runner_on_second_no_out"] < by_pressure["runners_corners_one_out"]
        and by_pressure["runners_corners_one_out"] < by_pressure["bases_loaded_no_out"]
        and by_pressure["bases_loaded_two_out"] < by_pressure["bases_loaded_no_out"]
        and by_pressure["jam_escape_high_leverage"] > by_pressure["runner_on_first_one_out"]
    )

    adjusted_metric_bounds_valid = all(
        -0.30 <= float(row["adjusted_inherited_runner_quality_score"]) <= 0.22
        and 0.12 <= float(row["adjusted_inherited_runner_k_rate"]) <= 0.38
        and 0.045 <= float(row["adjusted_inherited_runner_bb_rate"]) <= 0.17
        and 0.265 <= float(row["adjusted_inherited_runner_xwoba_allowed"]) <= 0.410
        and 0.0 <= float(row["inherited_runner_risk_score"]) <= 1.0
        and -0.10 <= float(row["strand_support_score"]) <= 0.20
        for row in rows
    )

    late_roles = {"closer", "setup", "high_leverage"}
    traffic_role_usage_valid = (
        all(row["selected_role"] in late_roles for row in rows if row["scenario_name"] in {"bases_loaded_no_out", "jam_escape_high_leverage"})
        and all(row["selected_role"] != "long_relief" for row in rows if row["scenario_name"] in {"bases_loaded_no_out", "jam_escape_high_leverage"})
        and all(row["selected_role"] != "closer" for row in rows if row["scenario_name"] == "clean_start")
    )

    risk_support_behavior_valid = (
        by_risk["bases_loaded_no_out"] > by_risk["clean_start"]
        and by_risk["bases_loaded_no_out"] > by_risk["runner_on_first_one_out"]
        and by_pressure["bases_loaded_no_out"] == max(by_pressure.values())
        and all(row["avg_strand_support"] is not None for row in scenario_rows)
    )

    selected_status_counts = _count(rows, "selected_availability_status")
    selected_role_counts = _count(rows, "selected_role")
    unavailable_selected_rate = round(selected_status_counts.get("unavailable", 0) / max(len(rows), 1), 4)

    high_traffic_rows = [row for row in rows if row["scenario_name"] in {"bases_loaded_no_out", "jam_escape_high_leverage", "runners_corners_one_out"}]
    high_traffic_late_role_rate = round(sum(1 for row in high_traffic_rows if row["selected_role"] in late_roles) / max(len(high_traffic_rows), 1), 4)

    checks = [
        {"check": "inherited_context_rows_available", "passed": len(fatigue_rows) == expected_fatigue_rows and len(rows) == expected_context_rows, "detail": f"{len(fatigue_rows)}/{expected_fatigue_rows}; {len(rows)}/{expected_context_rows}"},
        {"check": "scenario_coverage_valid", "passed": len(grouped_team) == expected_team_sides and all(len(v) == len(INHERITED_SCENARIOS) for v in grouped_team.values()), "detail": len(grouped_team)},
        {"check": "pressure_ordering_valid", "passed": pressure_ordering_valid, "detail": by_pressure},
        {"check": "adjusted_metric_bounds_valid", "passed": adjusted_metric_bounds_valid, "detail": adjusted_metric_bounds_valid},
        {"check": "traffic_role_usage_valid", "passed": traffic_role_usage_valid, "detail": selected_role_counts},
        {"check": "risk_support_behavior_valid", "passed": risk_support_behavior_valid, "detail": by_risk},
        {"check": "no_unavailable_selected", "passed": selected_status_counts.get("unavailable", 0) == 0, "detail": selected_status_counts},
        {"check": "candidate_mode_only", "passed": all(row.get("candidate_only") is True and row.get("analysis_only") is True for row in rows), "detail": True},
    ...
    strand_support_values = [float(row["strand_support_score"]) for row in rows]

    diagnosis = {
        "diagnosis": "candidate_bullpen_inherited_runner_context_analysis_complete",
        "fatigue_rows_analyzed": len(fatigue_rows),
        "inherited_context_rows_analyzed": len(rows),
        "expected_inherited_context_rows": expected_context_rows,
        "selected_role_counts": selected_role_counts,
        "selected_status_counts": selected_status_counts,
        "scenario_pressure": by_pressure,
        "scenario_risk": by_risk,
        "metrics": {
            "max_inherited_runner_risk_score": round(max(float(row["inherited_runner_risk_score"]) for row in rows), 4),
            "min_strand_support_score": round(min(strand_support_values), 4),
            "max_strand_support_score": round(max(strand_support_values), 4),
            "high_traffic_late_role_rate": high_traffic_late_role_rate,
        ...
        "recommended_next_layer": (
            "6BL_candidate_bullpen_depletion_sequence_prototype"
            if all(check["passed"] for check in checks)
            else "6BJ_patch_candidate_bullpen_inherited_runner_context_prototype"
        ),
    }
