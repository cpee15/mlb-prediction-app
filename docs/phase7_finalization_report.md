...
- Calendar frontend now uses `GET /matchups/calendar/schedule` for initial load.
...
| Phase 6 | Formula/simulation cache | #957 | Active/merge pending at time this report was updated |
...
|---|---|---|---|
| `GET /matchups/calendar/schedule` | Lightweight calendar/date-selector load and frontend fallback slate | Schedule-only snapshot | Shared calendar artifact/cache |
...
| `GET /matchups?date=<date>` | Full Daily Matchups | Matchups date cache or build | Matchups cache |
...
| `GET /daily-odds/models?date=<date>` | Daily Odds model layer | Daily odds build/cache | Daily odds cache |
| `GET /odds/draftkings/events?date=<date>` | Market context | Odds provider/cache | Odds provider/cache |
...
| Daily Matchups / HomePage | Full `/matchups?date=<date>` or error panel | Full `/matchups?date=<date>` first; if that fails, render schedule rows from `/matchups/calendar/schedule` | Implemented fallback; full model fields return when the heavy route succeeds |
...
| Daily Odds | One failed route in `Promise.all` failed the whole page | Uses `Promise.allSettled`; renders from successful routes and calendar fallback matchups | Implemented in `frontend/src/pages/DailyOddsPageFixed.jsx` |
...
| Daily Matchups / HomePage | Should display Model Projections where available or explicit fallback | Route/component verification plus frontend fallback metadata | Full model probability pending route availability; schedule fallback intentionally displays Pending |
...
| Daily Odds | Should prefer Model Projection probability where available | Uses projections where available; allSettled keeps page rendering when one route fails | Needs production verification |
